...
import { signInWithEmailAndPassword, signOut, sendPasswordResetEmail, updateEmail, updatePassword, EmailAuthProvider, reauthenticateWithCredential } from 'firebase/auth'
...
export default function AdminPage() {
  ...
  const [fName, setFName] = useState(''); const [fDesc, setFDesc] = useState(''); const [fPrice, setFPrice] = useState(''); const [fCat, setFCat] = useState('Starters'); const [fType, setFType] = useState('veg'); const [fBest, setFBest] = useState(false); const [fPhoto, setFPhoto] = useState('')
  const [selectedColor, setSelectedColor] = useState('#e23744')
  const [showChangeCreds, setShowChangeCreds] = useState(false)
  const [currentPass, setCurrentPass] = useState(''); const [newEmail, setNewEmail] = useState(''); const [newPass, setNewPass] = useState(''); const [credsError, setCredsError] = useState('')
  ...
  useEffect(() => {
    ...
    const unsubS = onSnapshot(doc(db, 'settings', 'special'), snap => { if (snap.exists()) { const d = snap.data(); setSpecial(d); setSelectedColor(d.color || '#e23744') } })
    return () => { unsub(); unsubS() }
  }, [user])
  ...
  const saveBanner = async () => {
    const data = { ...special, tag: document.getElementById('eBannerTag').value, title: document.getElementById('eBannerTitle').value, sub: document.getElementById('eBannerSub').value, color: selectedColor }
  ...
  const updateCreds = async () => {
    ...
    try {
      ...
      if (newPass) await updatePass(user, newPass)
      showToast('Credentials updated!'); setShowChangeCreds(false)
      setCurrentPass(''); setNewEmail(''); setNewPass('')
    } catch (e) { setCredsError('Current password is incorrect.') }
  }
      ...
      <div style={{ background: '#fff', borderRadius: 16, padding: '28px 24px', width: '100%', maxWidth: 360 }}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <button onClick={() => setShowPass(!showPass)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#93959f' }}>👁</button>
          </div>
        </div>
        ...
        <div style={{ textAlign: 'center', marginTop: 14 }}>
          <span onClick={() => { sendPasswordResetEmail(auth, email || '').then(() => showToast('Check your Gmail for reset link!')).catch(() => setLoginError('Enter your Gmail above first.')) }} style={{ fontSize: 13, color: '#e23744', cursor: 'pointer', textDecoration: 'underline' }}>Forgot password? Reset via Gmail OTP</span>
        </div>
    ...
    <div style={{ maxWidth: 480, margin: '0 auto', background: '#f2f2f2', minHeight: '100vh' }}>
      ...
      <div style={{ padding: 16 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#fff', borderRadius: 10, padding: '10px 14px', marginBottom: 16 }}>
          ...
          <button onClick={() => setShowChangeCreds(!showChangeCreds)} style={{ background: 'none', border: '1px solid #d4d5d9', borderRadius: 8, padding: '5px 10px', fontSize: 11, color: '#686b78', cursor: 'pointer' }}>Change Credentials</button>
        </div>

        {/* Change Credentials */}
        {showChangeCreds && (
          <div style={{ background: '#fff', borderRadius: 12, padding: 16, marginBottom: 16, border: '1.5px solid #fde0e2' }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#e23744', marginBottom: 12 }}>🔐 Change Credentials</div>
            <div style={{ fontSize: 12, color: '#93959f', marginBottom: 12 }}>A password reset link will be sent to your Gmail. Or enter new credentials below.</div>
            <button onClick={sendOTP} style={{ background: '#e23744', color: '#fff', border: 'none', padding: '10px 16px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', width: '100%', marginBottom: 12 }}>Send Reset Link to Gmail →</button>
            {resetSent && <div style={{ fontSize: 12, color: '#3d9142', textAlign: 'center', marginBottom: 10, fontWeight: 500 }}>✅ Reset link sent! Check your Gmail inbox.</div>}
          </div>
        )}
        ...
        {activeTab === 'special' && (
          <>
            ...
            <div style={{ background: '#fff', borderRadius: 12, padding: 16, marginBottom: 16 }}>
              ...
              {[['eBannerTag', 'Banner Tag', special.tag], ['eBannerTitle', 'Banner Title', special.title], ['eBannerSub', 'Banner Subtitle', special.sub]].map(([id, label, val]) => (
                <div key={id} style={{ marginBottom: 10 }}>
                  ...
                  <input id={id} defaultValue={val} style={{ width: '100%', padding: '10px 12px', border: '1.5px solid #d4d5d9', borderRadius: 8, fontSize: 13, outline: 'none', fontFamily: 'inherit' }} />
                </div>
              ))}
              ...
              <div style={{ borderRadius: 10, padding: '12px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: selectedColor, marginBottom: 12 }}>
                <div>
                  <div style={{ fontSize: 9, fontWeight: 600, letterSpacing: '1.5px', color: 'rgba(255,255,255,0.7)', textTransform: 'uppercase', marginBottom: 3 }}>Preview</div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: '#fff' }}>Banner Preview</div>
                </div>
